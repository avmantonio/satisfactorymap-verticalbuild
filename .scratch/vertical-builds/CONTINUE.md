...
- **Ticket:** [Height view chrome](issues/13-height-view-chrome.md) is specced. Do not re-grill dock, two Z widgets, or A↔A′ / B↔B′ flip chrome.
...
- **Job:** [Real-client skeleton](issues/16-real-client-skeleton.md) is specced in [specs/2-5d-first-cut.md](specs/2-5d-first-cut.md). Next HITL in this strand is **implementing** that spec in the current client (`height_view.js`), not another grill. Cut marks in that walk are projected AABB. Richer laterals were surveyed in [Survey cut laterals](issues/20-survey-cut-laterals.md) (resolved; does not block the AABB skeleton). Top-down occupancy drawing stays fog.
...
- **Job (14):** Spec only in that chat — implement relocate observation in a dedicated client chat, not a re-grill. Isolation Cut remains 19’s XY clip; placing retargets the same L-frame to the ghost (dest + pad, attenuated payload, floor of travelers, existing panel, no band drag).
...
- **Job:** Implementation of 16 against [specs/2-5d-first-cut.md](specs/2-5d-first-cut.md) in a dedicated chat. Chart further only when that work makes a question sharp (wasm headroom numbers, named-file). Adding from outside the cube stays Horizon fog.
...
- **Job:** Next HITL is **implement 16** (`height_view.js`) against that spec. [Height-driven edits](issues/14-height-driven-edits.md) is a dedicated client chat *after* the skeleton — same first-ship, not fused into cut 1.
