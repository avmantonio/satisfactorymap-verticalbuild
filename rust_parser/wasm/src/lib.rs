...
impl SaveSession {
    ...
    pub fn load(bytes: Vec<u8>, on_progress: &js_sys::Function) -> Result<SaveSession, JsError> {
        ...
        // 20 payload steps + the index build = BUILD_STEP_COUNT ticks (same
        // shape as Python's buildAll, plus the static spawners, mapLimits and
        // caves steps); one shared SaveScan for both.
        let mut build_progress = |current: u64, total: u64| call(2, current, total);
        let (payload_json, index) =
